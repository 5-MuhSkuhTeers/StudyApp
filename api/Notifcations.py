...
class AssigmentNotifcations:
    ...
    # if task due in many days- send email 24 hr before due; if task due in 24- send email 1 hr before due, if task due in 60 mins - send email right now before due
    def setNotifications(self):
        ...
        # duration is the dueDateTime - todayDateTime
        duration = self.convertTimedeltaToMin(dueDateTime.replace(second= 0, microsecond=0) - today)

        #check to see if it is due within 60 mins
        if duration < (60):
          inject = today.strftime("%a, %Y-%b-%d %H:%M:%S -0000")
        #check to see if it is due within the a day (24 hours) but not within 60 mins
        elif duration < (60*24):
          inject = (dueDateTime - timedelta(hours=1)).strftime("%a, %Y-%b-%d %H:%M:%S -0000")
        #check if it is not due with in (24 hours) and in 60 mins- send email 24 hours before due 
        else:
